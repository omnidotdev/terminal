...
const LOG_LEVEL_ENV: &str = "OMNI_TERMINAL_LOG_LEVEL";

pub fn setup_environment_variables(config: &terminal_backend::config::Config) {
    #[cfg(unix)]
    {
        let terminfo = match (
    ...
    std::env::set_var("TERM_PROGRAM", "omni-terminal");
...
fn setup_logs_by_filter_level(
    ...
    if log_file {
        ...
        log_file_path = log_dir_path.join("omni-terminal.log");
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(config_path) = write_config_path {
        ...
        terminal_backend::config::create_config_file(config_path);
        return Ok(());
    }

    let (mut config, config_error) = match terminal_backend::config::Config::try_load() {
        Ok(config) => (config, None),
        Err(err) => (terminal_backend::config::Config::default(), Some(err)),
    };
