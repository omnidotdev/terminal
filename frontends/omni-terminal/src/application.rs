...
use terminal_window::event_loop::{DeviceEvents, EventLoop};
#[cfg(target_os = "macos")]
use terminal_window::platform::macos::ActiveEventLoopExtMacOS;
#[cfg(target_os = "macos")]
...
impl Application<'_> {
    pub fn new<'app>(
        config: terminal_backend::config::Config,
        config_error: Option<terminal_backend::config::ConfigError>,
        ...
        let _ = configuration_file_updates(
            terminal_backend::config::config_dir_path(),
            event_proxy.clone(),
        );
...
impl ApplicationHandler<EventPayload> for Application<'_> {
            ...
            TerminalEventType::Terminal(TerminalEvent::CreateNativeTab(
                ...
                if let Some(route) = self.router.routes.get(&window_id) {
                    ...
                    let config = if working_dir_overwrite.is_some() {
                        terminal_backend::config::Config {
    ...
    fn hook_event(
        ...
        modifiers: &terminal_window::event::Modifiers,
    ) {
