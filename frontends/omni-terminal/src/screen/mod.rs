...
use terminal_backend::crosswords::pos::{Boundary, CursorState, Direction, Line};
use terminal_backend::crosswords::search::RegexSearch;
use terminal_backend::event::{ClickState, EventProxy, SearchState};
...
use terminal_window::event::Modifiers;
...
pub struct Screen<'screen> {
    ...
    hints_config: Vec<std::rc::Rc<terminal_backend::config::hints::Hint>>,
}

pub struct ScreenWindowProperties {
    pub size: terminal_window::dpi::PhysicalSize<u32>,
    pub scale: f64,
    pub raw_window_handle: RawWindowHandle,
    pub raw_display_handle: RawDisplayHandle,
    pub window_id: terminal_window::window::WindowId,
}
...
impl Screen<'_> {
    ...
    #[inline]
    pub fn process_key_event(&mut self, key: &terminal_window::event::KeyEvent) {
        ...
        if self.hint_state.is_active() {
            ...
            match key.logical_key {
                terminal_window::keyboard::Key::Named(
                    terminal_window::keyboard::NamedKey::Escape,
                ) => {
                ...
                terminal_window::keyboard::Key::Named(
                    terminal_window::keyboard::NamedKey::Backspace,
                ) => {
    ...
    pub fn split_right_with_config(&mut self, config: terminal_backend::config::Config) {
    ...
    pub fn update_highlighted_hints(&mut self) -> bool {
        ...
        if let Some(hint_match) = highlighted_hint {
            // Mark the hint range as damaged so it gets re-rendered
            {
                ...
                let hint_range = terminal_backend::selection::SelectionRange::new(
                    hint_match.start,
                    hint_match.end,
                    false,
                );
    ...
    fn find_hint_at_point(
        &self,
        terminal: &terminal_backend::crosswords::Crosswords<EventProxy>,
        point: terminal_backend::crosswords::pos::Pos,
        _modifiers: terminal_window::keyboard::ModifiersState,
    ...
    fn find_hyperlink_at_point(
        ...
        if let Some(hyperlink) = cell.hyperlink() {
            ...
            while start_col > terminal_backend::crosswords::pos::Column(0) {
            ...
            let hint_config = std::rc::Rc::new(terminal_backend::config::hints::Hint {
                regex: None,
                hyperlinks: true,
                post_processing: true,
                persist: false,
                action: terminal_backend::config::hints::HintAction::Command {
                    command: terminal_backend::config::hints::HintCommand::Simple(
                        "xdg-open".to_string(),
                    ),
                },
                mouse: terminal_backend::config::hints::HintMouse::default(),
                binding: None,
            });
            ...
            return Some(crate::hints::HintMatch {
                text: uri,
                start: terminal_backend::crosswords::pos::Pos::new(point.row, start_col),
                end: terminal_backend::crosswords::pos::Pos::new(point.row, end_col),
                hint: hint_config,
            });
        }
    ...
    fn find_regex_match_at_point(
        ...
        point: terminal_backend::crosswords::pos::Pos,
        regex: &regex::Regex,
        hint_config: std::rc::Rc<terminal_backend::config::hints::Hint>,
    ) -> Option<crate::hints::HintMatch> {
        let grid = &terminal.grid;
        ...
        for col in 0..grid.columns() {
            let cell = &grid[point.row][terminal_backend::crosswords::pos::Column(col)];
        ...
        for mat in regex.find_iter(line_text) {
            let start_col = terminal_backend::crosswords::pos::Column(mat.start());
            let end_col =
                terminal_backend::crosswords::pos::Column(mat.end().saturating_sub(1));
            ...
            if point.col >= start_col && point.col <= end_col {
                ...
                let (processed_start, processed_end) = if hint_config.post_processing {
                    self.hint_post_processing(
                        ...
                        start_col,
                        end_col,
                        terminal_backend::crosswords::pos::Line(point.row.0),
                    )
                ...
                return Some(crate::hints::HintMatch {
                    text: match_text,
                    start: terminal_backend::crosswords::pos::Pos::new(
                        point.row,
                        processed_start,
                    ),
    ...
    pub fn update_ime_cursor_position_if_needed(
        &mut self,
        window: &terminal_window::window::Window,
    ) {
    ...
    pub fn start_hint_mode(
        &mut self,
        hint: std::rc::Rc<terminal_backend::config::hints::Hint>,
    ) {
        ...
        match &hint_match.hint.action {
            HintAction::Action { action } => match action {
                ...
                HintInternalAction::Select => {
                    ...
                    let selection = terminal_backend::selection::SelectionRange::new(
                        hint_match.start,
                        hint_match.end,
    ...
    pub fn update_hint_state(&mut self) {
        ...
        if self.hint_state.is_active() {
            ...
            let matches: Vec<terminal_backend::crosswords::search::Match> = self
                .hint_state
                .matches()
                .iter()
            ...
            for (match_index, remaining_label) in visible_labels {
                if let Some(hint_match) = matches.get(match_index) {
                    ...
                    for (char_index, &label_char) in remaining_label.iter().enumerate() {
                        let position = terminal_backend::crosswords::pos::Pos::new(
    ...
    fn hint_post_processing(
        &self,
        terminal: &terminal_backend::crosswords::Crosswords<EventProxy>,
        start_col: terminal_backend::crosswords::pos::Column,
        end_col: terminal_backend::crosswords::pos::Column,
        row: terminal_backend::crosswords::pos::Line,
    ) -> Option<(
        terminal_backend::crosswords::pos::Column,
        terminal_backend::crosswords::pos::Column,
    )> {
        use terminal_backend::crosswords::grid::BidirectionalIterator;

        let grid = &terminal.grid;
        let start_pos = terminal_backend::crosswords::pos::Pos::new(row, start_col);
        let end_pos = terminal_backend::crosswords::pos::Pos::new(row, end_col);
