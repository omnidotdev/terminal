use terminal_backend::config::window::{Decorations, WindowMode};
use terminal_backend::config::Config;
use terminal_window::window::{
    CursorIcon, Fullscreen, Icon, ImePurpose, Window, WindowAttributes,
};

pub const LOGO_ICON: &[u8; 370070] =
...
pub const APPLICATION_ID: &str = "Omni Terminal";
...
pub fn create_window_builder(
    ...
    let mut window_builder = WindowAttributes::default()
        .with_title(title)
        .with_min_inner_size(terminal_window::dpi::LogicalSize {
    ...
    match config.window.decorations {
        ...
        Decorations::Transparent => {
            #[cfg(target_os = "macos")]
            {
                use terminal_window::platform::macos::WindowAttributesExtMacOS;
    ...
    {
        use terminal_window::platform::x11::WindowAttributesExtX11;
    ...
    {
        use terminal_window::platform::wayland::WindowAttributesExtWayland;
        let app_name = app_id.unwrap_or(APPLICATION_ID);
        window_builder = window_builder.with_name(app_name.to_lowercase(), app_name);
    }
    ...
    {
        use terminal_window::platform::windows::WindowAttributesExtWindows;
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        window_builder = window_builder
            .with_colorspace(config.window.colorspace.to_terminal_window_colorspace());
    ...
    match config.window.mode {
        ...
        _ => {
            window_builder =
                window_builder.with_inner_size(terminal_window::dpi::LogicalSize {
...
pub fn configure_window(winit_window: &Window, config: &Config) {
    ...
    // winit_window.set_ime_cursor_area(terminal_window::dpi::PhysicalPosition::new(500.0, 500.0), terminal_window::dpi::LogicalSize::new(400, 400));
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        use terminal_window::platform::macos::{OptionAsAlt, WindowExtMacOS};
    ...
    #[cfg(target_os = "windows")]
    {
        use terminal_backend::config::window::WindowsCornerPreference;
        use terminal_window::platform::windows::WindowExtWindows;
        ...
        if let Some(with_corner_preference) = &config.window.windows_corner_preference {
            let preference = match with_corner_preference {
                WindowsCornerPreference::Default => {
                    terminal_window::platform::windows::CornerPreference::Default
                }
                WindowsCornerPreference::DoNotRound => {
                    terminal_window::platform::windows::CornerPreference::DoNotRound
                }
                WindowsCornerPreference::Round => {
                    terminal_window::platform::windows::CornerPreference::Round
                }
                WindowsCornerPreference::RoundSmall => {
                    terminal_window::platform::windows::CornerPreference::RoundSmall
                }
            };
