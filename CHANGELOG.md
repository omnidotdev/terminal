...
- [#51](https://github.com/omnidotdev/terminal/pull/51) [`f0e9b41`](https://github.com/omnidotdev/terminal/commit/f0e9b4158d17da3f890533c5b0d996160cc7cd63) Thanks [@coopbri](https://github.com/coopbri)! - Add custom tab labels. Press `Ctrl+Shift+L` (`Cmd+Shift+L` on macOS) or bind the new `renametab` action to open a prompt and give the current tab a human-readable name. A custom label overrides the auto-derived title template and is not overwritten by process, working-directory, or OSC title changes. Committing an empty value clears the label and reverts the tab to its automatic title.
