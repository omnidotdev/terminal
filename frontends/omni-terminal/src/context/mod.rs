...
impl<T: terminal_backend::event::EventListener> Drop for Context<T> {
    fn drop(&mut self) {
...
pub struct ContextManagerConfig {
    ...
    pub title: terminal_backend::config::title::Title,
    pub keyboard: terminal_backend::config::keyboard::Keyboard,
}
...
pub fn create_dead_context<T: terminal_backend::event::EventListener>(
    event_proxy: T,
    window_id: WindowId,
    route_id: usize,
    rich_text_id: usize,
...
pub fn create_mock_context<
    T: terminal_backend::event::EventListener + Clone + std::marker::Send + 'static,
>(
    event_proxy: T,
    window_id: WindowId,
    rich_text_id: usize,
            ...
            Err(err_message) => {
                ...
                event_proxy.send_event(
                    TerminalEvent::ReportToAssistant(TerminalError {
                        ...
                        level: TerminalErrorLevel::Error,
                    }),
                    window_id,
                );
    ...
    match (old, new) {
        ...
        (Some(old_range), Some(new_range)) => {
            ...
            if old_range.is_block != new_range.is_block {
                return terminal_backend::event::TerminalDamage::Full;
            }
            ...
            terminal_backend::event::TerminalDamage::Partial(damaged_lines)
        }
    }
}
...
mod selection_damage_tests {
    ...
    fn test_no_selection_to_no_selection() {
        ...
        assert!(matches!(
            damage,
            terminal_backend::event::TerminalDamage::CursorOnly
        ));
    }
    ...
    fn test_selection_extend_down() {
        ...
        if let terminal_backend::event::TerminalDamage::Partial(lines) = damage {
