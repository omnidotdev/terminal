...
use terminal_window::platform::startup_notify::{
...
impl Route<'_> {
    ...
    pub fn begin_render(&mut self) {
        ...
        //     Option<HashMap<terminal_window::window::WindowId, (u64, std::time::Instant)>>,
        // > = Mutex::new(None);
    ...
    #[inline]
    pub fn report_error(&mut self, error: &TerminalError) {
        if error.report == TerminalErrorType::ConfigurationNotFound {
    ...
    pub fn has_key_wait(&mut self, key_event: &terminal_window::event::KeyEvent) -> bool {
        ...
        if self.path == RoutePath::Welcome && is_enter {
            terminal_backend::config::create_config_file(None);
            self.path = RoutePath::Terminal;
        }
...
pub struct Router<'a> {
    ...
    propagated_report: Option<TerminalError>,
    pub font_library: Box<terminal_backend::sugarloaf::font::FontLibrary>,
...
impl Router<'_> {
    pub fn new<'b>(
        fonts: terminal_backend::sugarloaf::font::SugarloafFonts,
        clipboard: Clipboard,
    ) -> Router<'b> {
        let (font_library, fonts_not_found) =
            terminal_backend::sugarloaf::font::FontLibrary::new(fonts);

        let mut propagated_report = None;

        if let Some(err) = fonts_not_found {
            propagated_report = Some(TerminalError {
                report: TerminalErrorType::FontsNotFound(err.fonts_not_found),
    ...
    #[inline]
    pub fn propagate_error_to_next_route(&mut self, error: TerminalError) {
    ...
    pub fn open_config_window(
        ...
        args.push(
            terminal_backend::config::config_file_path()
                .display()
                .to_string(),
        );
    ...
    pub fn create_window<'a>(
        ...
        event_proxy: EventProxy,
        config: &'a terminal_backend::config::Config,
...
impl<'a> RouteWindow<'a> {
    pub fn configure_window(&mut self, config: &terminal_backend::config::Config) {
